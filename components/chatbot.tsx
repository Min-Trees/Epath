...
export function Chatbot() {
  ...
  // -------------------------------------------------------------
  // Hide the floating chat FAB whenever the home-page hero section
  // is in the viewport — it would otherwise cover hero content.
  // We watch for the `.hero-section` element via IntersectionObserver
  // so the rule is automatic and doesn't require the homepage to
  // coordinate. We also bail out early on non-home routes so this
  // doesn't accidentally hide the FAB on `/about`, `/programs`, etc.
  // -------------------------------------------------------------
  const pathname = usePathname()
  const isHomePage = !pathname || pathname === '/' || pathname === '/vi' || pathname === '/en' || pathname === '/vi/' || pathname === '/en/'
  ...
  useEffect(() => {
    ...
    timerId = setTimeout(setupObserver, 50)
  ...
  // -------------------------------------------------------------
  // Periodically show "Bộ phận Học vụ đang online" badge
  // next to the chatbot button to attract user attention gracefully.
  // -------------------------------------------------------------
  const [showOnlineBadge, setShowOnlineBadge] = useState(false)

  useEffect(() => {
    if (isOpen || heroVisible) {
    ...
    const initialTimer = setTimeout(() => {
      triggerShow()
      // Then re-trigger every 22 seconds
  ...
  return (
    <>
      {/* Periodic "Bộ phận Học vụ đang online" notification pill */}
      <AnimatePresence>
        {showOnlineBadge && !isOpen && !heroVisible && (
          <motion.div
            initial={{ opacity: 0, scale: 0.85, x: 12, y: 0 }}
            animate={{ opacity: 1, scale: 1, x: 0, y: 0 }}
            exit={{ opacity: 0, scale: 0.85, x: 12, y: 0 }}
            transition={{ duration: 0.3, ease: 'easeOut' }}
            onClick={() => setIsOpen(true)}
            className="fixed bottom-[80px] right-4 sm:bottom-5 sm:right-[92px] lg:bottom-6 lg:right-[96px] z-[69] cursor-pointer select-none group"
            style={{ marginBottom: 'env(safe-area-inset-bottom, 0px)' }}
          >
            <div className="relative flex items-center gap-2.5 px-3.5 py-2 sm:px-4 sm:py-2.5 rounded-full bg-white border border-[#DEDDD6] shadow-lg shadow-[#1E3570]/12 hover:shadow-xl hover:border-[#8DC63F]/80 transition-all">
              {/* Desktop arrow pointing right to the FAB */}
              <span className="hidden sm:block absolute -right-1.5 top-1/2 -translate-y-1/2 w-3 h-3 bg-white border-t border-r border-[#DEDDD6] rotate-45 group-hover:border-[#8DC63F]/80 transition-colors" />

              {/* Mobile arrow pointing down to the FAB */}
              <span className="sm:hidden absolute -bottom-1.5 right-6 w-3 h-3 bg-white border-b border-r border-[#DEDDD6] rotate-45 group-hover:border-[#8DC63F]/80 transition-colors" />

              {/* Pulsing Green Indicator */}
              <span className="relative flex h-2.5 w-2.5 shrink-0">
                <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-[#8DC63F] opacity-75" />
                <span className="relative inline-flex rounded-full h-2.5 w-2.5 bg-[#8DC63F]" />
              </span>

              {/* Status text */}
              <span className="text-xs sm:text-sm font-bold text-[#1E3570] whitespace-nowrap group-hover:text-[#2E4A9E] transition-colors">
                {l === 'vi' ? 'Bộ phận Học vụ đang online' : 'Academic Department is online'}
              </span>
              ...
              <button
                ...
                className="ml-0.5 p-0.5 rounded-full text-[#5C6069] hover:text-[#1E3570] hover:bg-slate-100 transition-colors"
      ...
      {/* Chat Button – respects iOS safe-area so the bubble isn't hidden
          ...
          The button is hidden when the home-page hero section is in
          viewport so it doesn't cover hero content. */}
      <motion.button
        initial={false}
        animate={{
          scale: !heroVisible || isOpen ? 1 : 0,
          opacity: !heroVisible || isOpen ? 1 : 0,
          pointerEvents: !heroVisible || isOpen ? 'auto' : 'none',
        }}
        transition={{ duration: 0.25, ease: 'easeInOut' }}
        onClick={() => setIsOpen(!isOpen)}
        className="chat-fab fixed bottom-4 right-4 sm:bottom-5 sm:right-6 lg:bottom-6 lg:right-6 z-[70]"
        style={{
        ...
        <AnimatePresence mode="wait">
          {isOpen ? (
            ...
            <motion.div
              ...
              transition={{ duration: 0.2 }}
            >
              <MessageCircle className="w-6 h-6 sm:w-7 sm:h-7 text-white" strokeWidth={2.2} />
            </motion.div>
          )}
        </AnimatePresence>
        ...
        {isOpen && (
          ...
          >
            ...
            >
              ...
              {/* Status indicator ("Online" dot + label) intentionally
                  hidden per product request — the chat header is busy
                  enough without it, and the academic team is reachable
                  via the form / Zalo anyway. */}
              <button
                onClick={() => setIsOpen(false)}
                className="text-white/90 hover:text-white p-1 -mr-1 shrink-0 transition-colors"
      ...
      {/* The "Academic Department is online" floating label was removed
          per product request. The header now carries a synced chatbot
          icon (header-chatbot-icon) so users still have a visible chat
          affordance without the verbose status banner next to the FAB. */}
    </>
  )
}
